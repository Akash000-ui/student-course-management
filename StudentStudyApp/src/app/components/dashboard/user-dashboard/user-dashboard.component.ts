...
export class UserDashboardComponent implements OnInit, OnDestroy {
  ...
  onEnrollClick(course: Course): void {
    // Check if course is PAID - if so, redirect to payment instead
    if (this.isPaidCourse(course)) {
      console.log('Paid course detected. Redirecting to payment checkout for course:', course.id);
  ...
  /**
   * Check if a course is paid
   */
  private isPaidCourse(course: Course): boolean {
    try {
      ...
      if (typeof courseType === 'string') {
        return courseType.toUpperCase() === 'PAID';
      }
      ...
      if (typeof courseType === 'object' && courseTypeAny.name) {
        return courseTypeAny.name === 'PAID';
      }

      // Direct equality check
      return courseType === 'PAID';
    } catch (error) {
      console.error('Error checking if course is paid:', error);
      return false;
    }
  }
