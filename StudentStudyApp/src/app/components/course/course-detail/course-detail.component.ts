...
export class CourseDetailComponent implements OnInit, OnDestroy {
    ...
    this.courseService.getCourseById(this.courseId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (response) => {
        ...
        if (response.success && response.data) {
          ...
          console.log('Course Type:', this.course.courseType);
          console.log('Course Price:', this.course.price);
          console.log('Is Paid Course:', this.isPaidCourse());
          // Load category name
  ...
  //   if (!this.course) {
  //     console.log('isPaidCourse: course is null/undefined');
  //     return false;
  //   }

  //   const courseType = this.course.courseType;
  //   console.log('isPaidCourse check - full course object:', JSON.stringify(this.course));
  //   console.log('isPaidCourse check - courseType value:', courseType, 'typeof:', typeof courseType);

  //   // Handle null/undefined
  //   if (!courseType) {
  //     console.log('isPaidCourse: courseType is null/undefined');
  //     return false;
  //   }

  //   // Handle both string and potential object formats
  //   if (typeof courseType === 'string') {
  //     const isPaid = courseType.toUpperCase() === 'PAID';
  //     console.log('isPaidCourse result (string comparison):', isPaid);
  //     return isPaid;
  //   }

  //   // Handle enum-like objects that might have a name property
  //   if (typeof courseType === 'object' && courseType.name) {
  //     const isPaid = courseType.name === 'PAID';
  //     console.log('isPaidCourse result (object.name comparison):', isPaid);
  //     return isPaid;
  //   }

  //   const result = courseType === 'PAID';
  //   console.log('isPaidCourse result (final):', result);
  //   return result;
  // }
  ...
  isPaidCourse(): boolean {
    try {
      ...
      const courseType = this.course.courseType;
      console.log('isPaidCourse check - courseType:', courseType, 'typeof:', typeof courseType);
      ...
      // If it's a string, convert to uppercase and compare
      if (typeof courseType === 'string') {
        const result = courseType.toUpperCase() === 'PAID';
        console.log('isPaidCourse: string comparison result:', result);
        return result;
      }

      // If it's an object with a name property (enum serialization)
      const courseTypeAny = courseType as any;
      if (typeof courseType === 'object' && courseTypeAny.name) {
        const result = courseTypeAny.name === 'PAID';
        console.log('isPaidCourse: object.name comparison result:', result);
        return result;
      }

      // Direct equality check
      const result = courseType === 'PAID';
      console.log('isPaidCourse: direct comparison result:', result);
      return result;
    } catch (error) {
      console.error('Error in isPaidCourse:', error);
      return false;
    }
  }
   ...
   */
  enrollInCourse(): void {
    if (!this.user || this.enrolling || !this.course) return;

    // Check if course is PAID - if so, redirect to payment
    if (this.isPaidCourse()) {
      console.log('Paid course detected. Redirecting to payment checkout for course:', this.courseId);
    ...
    this.enrollmentService.enrollInCourse(request)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          console.log('Enrollment error:', errorMessage);

          // If it's a paid course error, redirect to payment
          if (errorMessage.includes('paid course') || errorMessage.includes('payment')) {
            console.log('Paid course payment required - redirecting to checkout');
            this.showError('This is a paid course. Redirecting to payment...');
            setTimeout(() => {
              this.router.navigate(['/payment/checkout'], {
                queryParams: { courseId: this.courseId }
              });
            ...
            this.showError(errorMessage || 'Failed to enroll in course');
          }
        }
      });
  }
